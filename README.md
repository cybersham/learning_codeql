...
```ql
/**
 * Track tainted data flowing from Java user input to C native calls
 * This query identifies String parameters passed to native functions
 * that could be tainted with user-controlled data.
 */

import java
import cpp

class UserInputSource extends DataFlow::Source {
  UserInputSource() {
    // Java: HttpServletRequest parameters
    exists(MethodAccess ma |
      ma.getMethod().getName() = "getParameter" and
      this = DataFlow::exprNode(ma)
    )
    // Java: Request body parsing
    or exists(MethodAccess ma |
      ma.getMethod().getName().matches("parse%") and
      this = DataFlow::exprNode(ma)
    )
  }
}

class NativeMethodSink extends DataFlow::Sink {
  NativeMethodSink() {
    // C/C++: File write operations
    exists(FunctionCall fc |
      fc.getTarget().getName().matches("fopen|open|write|fprintf") and
      this = DataFlow::exprNode(fc.getArgument(_))
    )
  }
}

query predicate vulnerableTaintFlow(
  UserInputSource source,
  NativeMethodSink sink
) {
...
```yaml
name: "CodeQL: Cross-Language Analysis"
...
jobs:
  analyze:
    ...
    steps:
      ...
      - name: Custom Query: Cross-Language Taint Flow
        uses: github/codeql-action/analyze@v3
        with:
          queries: ./queries/java-to-c-taint-flow.ql
```

## Vulnerable Example

### Java Code: Accepting User Input
```java
// src/java/VulnerableBackend.java
@RestController
public class FileProcessor {
    private native void processFile(String filePath);

    @PostMapping("/process")
    public ResponseEntity<?> processUserFile(@RequestParam String filename) {
        // ❌ VULNERABLE: Tainted user input passed directly to native code
        processFile(filename);  // This filename could be "../../etc/passwd"
...
```c
// src/c/native_extension.c
#include <stdio.h>
#include <jni.h>

JNIEXPORT void JNICALL 
Java_com_example_FileProcessor_processFile(JNIEnv *env, jobject obj, jstring filePath) {
    const char *path = (*env)->GetStringUTFChars(env, filePath, NULL);
    
    // ❌ VULNERABLE: Path injection - user controls the file location
    FILE *file = fopen(path, "w");  // Any path: "/tmp/malicious", "../../etc/shadow"
    if (file) {
        fprintf(file, "Malicious content\n");
        fclose(file);
    }
...
```
User Request: POST /process?filename=../../etc/passwd
    ↓
Java Backend: @RequestParam String filename
    ↓
processFile(filename) [JNI call]
    ↓
C Extension: fopen(path, "w")  ← ❌ DETECTED BY CUSTOM QUERY
    ↓
...
```java
// ✅ SAFE: Validate before passing to native code
@PostMapping("/process")
public ResponseEntity<?> processUserFile(@RequestParam String filename) {
    // Validate: Only alphanumeric and specific extensions
    if (!filename.matches("^[a-zA-Z0-9._-]+$") || !filename.endsWith(".dat")) {
        throw new IllegalArgumentException("Invalid filename");
    }
    
    // Use safe directory prefix
    String safePath = "/var/data/uploads/" + filename;
    processFile(safePath);
    return ResponseEntity.ok("Processed");
}
```
...
| Query | Purpose | Language |
|-------|---------|----------|
| `java-to-c-taint-flow.ql` | Detect tainted data crossing JNI boundary | Java/C |
| `path-injection-detector.ql` | Find path injection vulnerabilities | C/C++ |
| `jni-boundary-analysis.ql` | Analyze all JNI call sites | Java |
